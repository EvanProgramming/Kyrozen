...
class ProblemDiscoveryAgent(BaseAgent):
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            ...
            "Available tools:\n" + tools_text + "\n\n"
            "Rules:\n"
            "- Always respond in the same language as the user's latest message, including question labels and options.\n"
            ...
            "- Ask only 1 focused follow-up question at a time. Never dump a long questionnaire.\n"
            "- Ask at most 4 follow-up questions total, always one at a time.\n"
            "- Ask only about the user's real problem, affected people, current workaround, desired outcome, and must-have functions. Never ask the user to choose frameworks, databases, protocols, or other implementation details.\n"
            ...
            "  The JSON must have keys: question, options (array of {label, value}), allow_other (boolean).\n"
            "  Example: ```kyrozen-question\\n{\\\"question\\\":\\\"Who is the main user?\\\",\\\"options\\\":[{\\\"label\\\":\\\"Myself\\\",\\\"value\\\":\\\"Myself\\\"}],\\\"allow_other\\\":true}\\n```\n"
