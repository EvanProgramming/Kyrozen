...
| 客户端更新                                 | 检测到新版本后弹窗提示手动更新          | 平衡及时性与用户控制权                                               |
...
| 依赖安装                                   | 自动执行，展示来源但不确认              | 流畅，信任 Agent 判断                                                |
| 执行计划                                   | 顶部横幅展示后自动执行                  | 用户可见但无需手动确认                                               |
| 多项目管理                                 | 单窗口，左侧项目列表切换                | 类似 VS Code 工作区                                                  |
...
**订阅与配额**：云端在每次模型调用前检查用户订阅套餐和剩余额度，返回配额不足错误时，本地 Agent 将错误信息展示给用户并引导升级。
...
- **对话流**：核心交互区，展示用户消息、AI 回复、工具调用摘要、文件变更摘要。
- **底部输入框**：用户输入消息，支持上传文件/截图。
- **辅助功能**：
  - 双击文件树中的文件可打开简单内置编辑器。
...
| 无客户端用户 | 暂时不支持 |
| 错误上报 | 用户确认后上传脱敏日志 |
| 文件监控同步 | 同步关键文件内容（如 package.json）到云端 |
