...
Kyrozen 桌面客户端是 Kyrozen 网站部署后在用户本地执行 Agent 的配套应用。它负责操作本地文件、运行终端命令、执行 Git 操作，以及在用户确认设备和工具链后执行硬件实验流程等与本地环境强相关的任务，同时通过 WebSocket 与云端保持实时同步。

硬件边界：当前第二阶段首轮硬件链路以 Arduino CLI/PlatformIO 和 ESP32 串口抽象为范围。没有检测到真实板卡时，编译/上传/串口观察和实物验收必须保持 `BLOCKED`；Fake Transport 和模拟器只证明软件协议路径，不能替代板卡行为、拔插恢复或用户确认。本版本不臆造 BLE/GATT、MTU、OTA 或确认协议。

## 1. 定位

- 网站端：处理无需本地资源的阶段，例如问题发现、市场调研、产品规划。
- 桌面端：处理需要本地资源的高权限阶段，例如软件开发、串口硬件实验和测试验证；完整硬件验收仍以真实设备证据为准。
